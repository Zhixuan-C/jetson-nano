...
class ARNode:
    def __init__(self, name):
        ...
        self.result_publisher = rospy.Publisher(self.name + '/image_result', Image, queue_size=1)  # 发布最终图像(publish the final image)
        self.enter_srv = rospy.Service(self.name + '/enter', Trigger, self.enter_srv_callback)  # 进入发玩法服务(enter the game service)
        self.exit_srv = rospy.Service(self.name + '/exit', Trigger, self.exit_srv_callback)  # 退出玩法服务(exit the game service)
        self.heart = Heart(self.name + '/heartbeat', 5, lambda _: self.exit_srv_callback(None))  # 心跳包(heartbeat package)
        self.set_model_srv = rospy.Service(self.name + '/set_model', SetString, self.set_model_srv_callback)  # 设置模型服务(set the model service)
    ...
    def enter_srv_callback(self, msg):
        ...
        with self.lock:
            self.obj = None
            self.target_model = None
            depth_camera = rospy.get_param('/depth_camera/camera_name', 'camera')  # 获取摄像头名称(get the camera name)
